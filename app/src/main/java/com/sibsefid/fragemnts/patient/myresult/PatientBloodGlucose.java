 ...
 */
public class PatientBloodGlucose extends Fragment {
    ...
    private List<GetPatientBloodGlucoseModel.DataBean> pGlucoseDetails;


    Callback<GetPatientBloodGlucoseModel> detailsCallback = new Callback<GetPatientBloodGlucoseModel>() {
        @Override
        public void onResponse(Call<GetPatientBloodGlucoseModel> call, Response<GetPatientBloodGlucoseModel> response) {
            if (getActivity() != null) {
                showProgress(false);
                if (response.isSuccessful()) {
                    if (response.body().isSuccess() && response.body().getData() != null && response.body().getData().size() > 0) {
                        if (response.body().getData() != null) {
                            pGlucoseDetails = response.body().getData();
        ...
        @Override
        public void onFailure(Call<GetPatientBloodGlucoseModel> call, Throwable t) {
            if (getActivity() != null) {
                showProgress(false);
            }
        }
    ...
    public PatientBloodGlucose() {
        // Required empty public constructor
    }

    private void updateUi() {

        // listMember.setAdapter(new PatientBloodGlucoseAdapter(pGlucoseDetails));
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_blood_glucose, container, false);

        initiateView(view);

        if (Utils.isDeviceOnline(getActivity())) {
            showProgress(true);
            //RestAdapter.getAdapter().GetPatientBloodGlucose(details.getUser_seq()).enqueue(detailsCallback);
        }
    ...
    private void initiateView(View view) {
        ...
        // listMember = (RecyclerView) view.findViewById(R.id.listMember);
       ...
       /* DisplayMetrics displaymetrics = new DisplayMetrics();
        ...
        String url = ApiUrls.BASE_URL_WebView+"blood_glucose.aspx?PatientId=" + details.getUser_seq();
        web_disply.loadUrl(url);
        web_disply.setWebViewClient(new WebViewClient() {
                                        @Override
                                        public void onPageFinished(WebView view, String url) {
                                            super.onPageFinished(view, url);
                                            // Toast.makeText(getActivity(),"PageLoadinginish",Toast.LENGTH_SHORT).show();
                                            showProgress(false);
                                        }
                                    }
        );
