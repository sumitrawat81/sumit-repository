...
public class HeartRateDetailFragment extends Fragment {
    ...
    Callback<PatientHeartRateModel> detailsCallback = new Callback<PatientHeartRateModel>() {
        @Override
        public void onResponse(Call<PatientHeartRateModel> call, Response<PatientHeartRateModel> response) {
            if (getActivity() != null) {
                showProgress(false);
                if (response.isSuccessful()) {
                    if (response.body().isSuccess() && response.body().getData() != null && response.body().getData().size() > 0) {
                        if (response.body().getData() != null) {
                            pHeartRateDetails = response.body().getData();
        ...
        @Override
        public void onFailure(Call<PatientHeartRateModel> call, Throwable t) {
            if (getActivity() != null) {
                showProgress(false);
            }
        }
    ...
    private void updateUi() {
        ...
        // listMember.setAdapter(new HeartRateDetailAdapter(pHeartRateDetails));
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.heart_rate_detail_fragment, container, false);
        ...
        if (Utils.isDeviceOnline(getActivity())) {
            showProgress(true);
            //  RestAdapter.getAdapter().GetPatientHeartRate(details.getUser_seq()).enqueue(detailsCallback);
        }
    ...
    private void initiateView(View view) {
        ...
        String url = ApiUrls.BASE_URL_WebView+"heart_rate.aspx?patientid=" + details.getUser_seq();
