 ...
 */
public class WaistCircumferenceFragment extends Fragment {
    ...
    private List<GetPatientWaistModel.DataBean> pWasitDetails;


    Callback<GetPatientWaistModel> detailsCallback = new Callback<GetPatientWaistModel>() {
        @Override
        public void onResponse(Call<GetPatientWaistModel> call, Response<GetPatientWaistModel> response) {
            if (getActivity() != null) {
                showProgress(false);
                if (response.isSuccessful()) {
                    if (response.body().isSuccess() && response.body().getData() != null && response.body().getData().size() > 0) {
                        if (response.body().getData() != null) {
                            pWasitDetails = response.body().getData();
        ...
        @Override
        public void onFailure(Call<GetPatientWaistModel> call, Throwable t) {
            if (getActivity() != null) {
                showProgress(false);
            }
        }
    ...
    public WaistCircumferenceFragment() {
    ...
    private void updateUi() {
        ...
        // listMember.setAdapter(new WaistCircumferenceAdapter(pWasitDetails));
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.waist_circumference_fragment, container, false);

        initiateView(view);

        if (Utils.isDeviceOnline(getActivity())) {
            showProgress(true);
            //  RestAdapter.getAdapter().GetPatientWaist(details.getUser_seq()).enqueue(detailsCallback);
        }
    ...
    private void initiateView(View view) {
        ...
        String url = ApiUrls.BASE_URL_WebView+"waist_circumference.aspx?patientid=" + details.getUser_seq();
