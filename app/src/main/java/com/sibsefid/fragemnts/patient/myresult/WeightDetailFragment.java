...
public class WeightDetailFragment extends Fragment {
    ...
    private List<GetPatientWeightModel.DataBean> pWEightDetails;


    Callback<GetPatientWeightModel> detailsCallback = new Callback<GetPatientWeightModel>() {
        @Override
        public void onResponse(Call<GetPatientWeightModel> call, Response<GetPatientWeightModel> response) {
            if (getActivity() != null) {
                showProgress(false);
                if (response.isSuccessful()) {
                    if (response.body().isSuccess() && response.body().getData() != null && response.body().getData().size() > 0) {
                        if (response.body().getData() != null) {
                            pWEightDetails = response.body().getData();
        ...
        @Override
        public void onFailure(Call<GetPatientWeightModel> call, Throwable t) {
            if (getActivity() != null) {
                showProgress(false);
            }
        }
    ...
    private void updateUi() {
        ...
        //  listMember.setAdapter(new WeightDetailFragmentAdapter(pWEightDetails));
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.weight_detail_fragment, container, false);

        initiateView(view);
        if (Utils.isDeviceOnline(getActivity())) {
            showProgress(true);
            //  RestAdapter.getAdapter().GetPatient_Weight(details.getUser_seq()).enqueue(detailsCallback);
        }
        return view;
    }
    ...
    private void initiateView(View view) {
        ...
        String url = ApiUrls.BASE_URL_WebView+"weight.aspx?patientid=" + details.getUser_seq();
