 ...
 */
public class EcgDetailFragment extends Fragment {
    ...
    private List<PatientEcgDetailModel.DataBean> pEcgDetails;


    Callback<PatientEcgDetailModel> detailsCallback = new Callback<PatientEcgDetailModel>() {
        @Override
        public void onResponse(Call<PatientEcgDetailModel> call, Response<PatientEcgDetailModel> response) {
            if (getActivity() != null) {
                showProgress(false);
                if (response.isSuccessful()) {
                    if (response.body().isSuccess() && response.body().getData() != null && response.body().getData().size() > 0) {
                        if (response.body().getData() != null) {
                            pEcgDetails = response.body().getData();
        ...
        @Override
        public void onFailure(Call<PatientEcgDetailModel> call, Throwable t) {
            if (getActivity() != null) {
                showProgress(false);
            }
        }
    ...
    private void updateUi() {
        ...
        // listMember.setAdapter(new EcgDetailFragmentAdapter(pEcgDetails));
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.ecg_detail_fragment, container, false);

        initiateView(view);

        if (Utils.isDeviceOnline(getActivity())) {
            showProgress(true);
            // RestAdapter.getAdapter().GetPatientECG(details.getUser_seq()).enqueue(detailsCallback);
        }
    ...
    private void initiateView(View view) {
        ...
        String url = ApiUrls.BASE_URL_WebView+"ecg.aspx?patientid=" + details.getUser_seq();
