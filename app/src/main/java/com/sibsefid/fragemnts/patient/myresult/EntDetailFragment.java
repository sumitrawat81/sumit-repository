 ...
 */
public class EntDetailFragment extends Fragment {
    ...
    Callback<PatientEntDetailModel> detailsCallback = new Callback<PatientEntDetailModel>() {
        @Override
        public void onResponse(Call<PatientEntDetailModel> call, Response<PatientEntDetailModel> response) {
            if (getActivity() != null) {
                showProgress(false);
                if (response.isSuccessful()) {
                    if (response.body().isSuccess() && response.body().getData() != null && response.body().getData().size() > 0) {
                        if (response.body().getData() != null) {
                            pEntDetails = response.body().getData();
        ...
        @Override
        public void onFailure(Call<PatientEntDetailModel> call, Throwable t) {
            if (getActivity() != null) {
                showProgress(false);
            }
        }
    ...
    public EntDetailFragment() {

    }

    private void updateUi() {

        // listMember.setAdapter(new EntDetailFragmentAdapter(pEntDetails));
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.ent_detail_fragment, container, false);

        initiateView(view);

        if (Utils.isDeviceOnline(getActivity())) {
            showProgress(true);
            //  RestAdapter.getAdapter().GetPatientENT(details.getUser_seq()).enqueue(detailsCallback);
        }
    ...
    private void initiateView(View view) {
        ...
        String url = ApiUrls.BASE_URL_WebView+"ent.aspx?patientid=" + details.getUser_seq();
