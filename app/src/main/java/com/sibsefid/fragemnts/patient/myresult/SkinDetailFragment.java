...
public class SkinDetailFragment extends Fragment {
    ...
    Callback<PatientSkinDetailModel> detailsCallback = new Callback<PatientSkinDetailModel>() {
        @Override
        public void onResponse(Call<PatientSkinDetailModel> call, Response<PatientSkinDetailModel> response) {
            if (getActivity() != null) {
                showProgress(false);
                if (response.isSuccessful()) {
                    if (response.body().isSuccess() && response.body().getData() != null && response.body().getData().size() > 0) {
                        if (response.body().getData() != null) {
                            pSkinDetails = response.body().getData();
        ...
        @Override
        public void onFailure(Call<PatientSkinDetailModel> call, Throwable t) {
            if (getActivity() != null) {
                showProgress(false);
            }
        }
    ...
    private void updateUi() {
        ...
        //listMember.setAdapter(new SkinDetailFragmentAdapter(pSkinDetails));
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.skin_detail_fragment, container, false);

        initiateView(view);

        if (Utils.isDeviceOnline(getActivity())) {
            showProgress(true);
            //  RestAdapter.getAdapter().GetPatientSkinDetail(details.getUser_seq()).enqueue(detailsCallback);
        }
    ...
    private void initiateView(View view) {
        ...
        String url = ApiUrls.BASE_URL_WebView+"skin.aspx?patientid=" + details.getUser_seq();
